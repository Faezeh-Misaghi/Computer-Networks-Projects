    ...
    while (true) {
        ...
        if (calculated_checksum == received_pkt.header.checksum) {
            ...
            if (received_pkt.header.seq_num == expected_seq_num) {
                ...
                outFile.flush();
                cout << "Received and wrote seq_num: " << received_pkt.header.seq_num << endl;
                expected_seq_num++; 
            } else {
                cout << "Duplicate packet seq_num: " << received_pkt.header.seq_num << ". Sending ACK again." << endl;
            }

            Packet ack_pkt;
            ack_pkt.header.seq_num = 0;
            ack_pkt.header.ack_num = received_pkt.header.seq_num;
            ...
            cout << "Packet corrupted! Checksum mismatch. Dropping" << endl;
        }
    }
