...
int main(int argc, char* argv[]) {
    ...
    while (base < total_packets) {
        ...
        if (bytes_rcvd > 0) {
            ...
            if ((ack_pkt.header.flags & FLAG_ACK) && calculate_checksum(ack_pkt) == ack_pkt.header.checksum) {
                ...
                else if (ack_num == (uint32_t)base) {
                    dup_ack_count++;
                    if (dup_ack_count == 3) {
                        events_file << get_time_elapsed() << " - EVENT: FAST RETRANSMIT occurred at base " << base << "\n";
                        total_fast_retransmits++;
    ...
    cout << "\nTransfer Complete (Bonus Phase Enabled)" << endl;
    ...
    cout << "Total timeout events: " << total_timeouts << endl;
    cout << "Total FAST RETRANSMITS: " << total_fast_retransmits << endl;
